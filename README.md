...
Plan, approve, publish, and explain multi-channel campaigns from one evidence trail.
...
npm install
npm link
export SIGNALDECK_TOKEN="a-scoped-workspace-token"
export SIGNALDECK_URL="https://cloud.getsupers.com"
signaldeck actions
signaldeck workspace
signaldeck page '{"limit":50,"state":"active","search":"Title prefix"}'
signaldeck detail '00000000-0000-4000-8000-000000000001'
signaldeck action channel-binding-preview '{"provider":"federated-social","accountRef":"brand-main","capabilities":["text","image"]}'
```
...
export SIGNALDECK_TOKEN="a-scoped-workspace-token"
export SIGNALDECK_URL="https://cloud.getsupers.com"
export SIGNALDECK_WEB_KEY="a-separate-random-browser-key"
npm start
```
...
```bash
docker build -t signaldeck:0.3.1 .
...
```json
{
  "mcpServers": {
    "signaldeck": {
      "command": "signaldeck-mcp",
      "env": {
        "SIGNALDECK_TOKEN": "a-scoped-workspace-token",
        "SIGNALDECK_URL": "https://cloud.getsupers.com"
      }
    }
  }
}
...
Then point `SIGNALDECK_URL` at the self-hosted control-plane origin. All products may share the same backend and PostgreSQL cluster while the backend preserves workspace and module boundaries.

## Typed action catalogue

| Action ID | Capability | Token scope | Operation |
|---|---|---|---|
| `channel-binding-preview` | Preview channel binding | `read` | `read` |
| `campaign-draft` | Create campaign brief | `write` | `command` |
| `content-variants-propose` | Propose cited content variants | `ai` | `ai` |
| `campaign-approve` | Approve campaign snapshot | `write` | `command` |
| `post-schedule` | Schedule approved post | `write` | `command` |
| `publication-dispatch` | Dispatch scheduled publication | `write` | `command` |
| `metrics-import` | Import channel metrics | `write` | `command` |
